...
function publishTable(): Promise<void> {
  return new Promise((resolve, reject) => {
    const socket = new WebSocket(new URL("ctl", MOCK).href.replace(/^http/, "ws"));
    socket.once("open", () => socket.send(JSON.stringify({ op: "raw",
      frame: { type: "reply", cmd: "get_tool_table", ok: true, tools } })));
...
test("a missing bundled asset is reported and can be retried", async ({ page }) => {
  await page.route("**/examples/tools/fusion-freecad.json", route => route.fulfill({ status: 404 }));
  await page.goto(MOCK);
  await expect(page.locator('fieldset[data-gate="armed"]').first()).not.toBeDisabled();
  await page.getByRole("button", { name: "Tools", exact: true }).click();
  const button = page.getByRole("button", { name: "Examples", exact: true });
  await button.click();
  await expect(page.getByText("Example library unavailable (HTTP 404)")).toBeVisible();
