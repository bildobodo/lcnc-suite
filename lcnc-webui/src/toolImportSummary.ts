...
export function summarizeToolImport(tools: { source_format?: unknown; is_example?: unknown }[]) {
  ...
  const zeroOffsets = tools.length > 0 && tools.every(t => t.source_format === "freecad");
  const sourceLabel = freecad && fusion ? "Fusion 360 + FreeCAD" : freecad ? "FreeCAD" : "Fusion 360";
  return {
    freecad, fusion, isExample, sourceLabel,
    replacementNotice: isExample
      ? "Z offsets use nominal example lengths for simulation. Measure installed tools before machining."
      : zeroOffsets
    ...
    resultNotice: isExample
      ? "Z offsets initialized from nominal example lengths for simulation."
      : zeroOffsets
