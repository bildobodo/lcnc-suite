...
<template>
  ...
  >
    ...
    <div class="inputField nkDisplay stack-micro">
      <div class="nkExpr">{{ displayExpr }}</div>
      <div class="nkPreview" :class="{ invalid: previewInvalid }">
        {{ previewText }}&nbsp;
      </div>
    </div>
    <div class="nkRows">
      <!-- Key grid: 5 columns × 4 rows -->
      <div class="nkGrid">
        <!-- Row 1 -->
      ...
      <!-- Actions column — same key types as the G-code keyboard's ops
           column (numOp + primary numKey), so both strips look alike. -->
      <div class="nkActions">
        <MachineBtn type="numOp" class="nkKey" @pointerdown.prevent="press($event, cancel)" @contextmenu.prevent>Cancel</MachineBtn>
        <MachineBtn type="numEq" class="nkKey" :disabled="result === null" @pointerdown.prevent="press($event, evalExpr)" @contextmenu.prevent>═</MachineBtn>
        <MachineBtn type="numKey" variant="primary" class="nkKey" :disabled="result === null && !!expr.trim()" @pointerdown.prevent="press($event, confirm)" @contextmenu.prevent>OK</MachineBtn>
      </div>
    </div>
  </div>
</template>
...
/* Display box: .inputField provides the visual (bg, border, radius,
   tabular-nums) — only layout is added here. */
.nkDisplay {
...
/* Same row/grid pattern as GcodeKeypadStrip (gkRows/gkDigits/gkOps):
   stretch-aligned flex row, tight gaps inside a key cluster, 1fr rows so
   the clusters fill the strip height in landscape and fall back to
   natural key height in portrait (auto-height sections). */
.nkRows {
  display: flex;
