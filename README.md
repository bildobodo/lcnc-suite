...
- **Session-bound**: one gateway process serves exactly one LinuxCNC instance (linuxcncsvr pid + kernel start time). When that instance ends the gateway exits (the launcher then returns to LinuxCNC's Cleanup) instead of re-attaching to whatever LinuxCNC starts next; `hal_watchdog.py` / `hal_reader.py` admit only a gateway whose hello names their own instance (`session_bind.py`). `WEBUI_ALLOW_REBIND=1` restores the old reconnect-across-restarts behaviour for dev harnesses only
...
**Startup order does not matter**: The gateway and LinuxCNC can start in any order. The gateway binds to the LinuxCNC instance on its first NML connect and only then dials the webui-safety / webui-reader sockets, opening each connection with a hello that names that instance and its own pid; the helpers verify both (kernel `SO_PEERCRED`) and answer `welcome`, or `rejected` with a reason (`no_hello` for a pre-binding gateway, `stale_instance`, `duplicate_gateway` while another gateway is attached, `unbound_gateway`, `instance_ended`) — logged as `[WD] REJECTED …` / `[READER] REJECTED …` and shown in the gateway's safety-chain banner. A rejected connect never touches the admitted gateway's pins. Helpers that predate this protocol never answer; the gateway keeps heartbeating and shows the banner until LinuxCNC is restarted with the current helpers. `webui-hb-latch` powers up **Faulted** (`estop_latch` semantics) — the operator's first E-Stop Reset clears it as part of the normal power-up flow, and the gateway audits a boot-faulted latch (`safety.latch_faulted_on_connect`) instead of showing the trip banner. `connected`/`heartbeat` default FALSE until the gateway connects. `hal_watchdog.py` skips its first `hb-ok-in` read so the initial FALSE of `oneshot.0.out` (before the first heartbeat) doesn't register as a phantom trip.
