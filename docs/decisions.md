...
## 2026-09-16 — Merge the TWP foundation into development; guided setup follows

**Decision (user approved).** Merge `feat/twp` at `f221763` into
`development` as the technical-foundation milestone. The reviewed code
findings are resolved. Operator workflow completion is a separate milestone;
this merge does not mark M-05/M-06 passed or authorize promotion to `main`.

**Merge boundary.** `development` at `bd660c2` is already an ancestor of
the feature. The product source therefore matches the reviewed feature
without code-conflict resolutions. Exclude the accidentally tracked
`lcnc-gateway/.venv` absolute symlink: it points at the development checkout's
own environment and must remain a local, ignored installation. Preserve that
existing environment and local parity-run artifacts during integration.

**Validation.** Reuse the independent results on the unchanged `f221763`
product tree: 803 frontend unit tests, all 24 committed browser tests,
production build, ESLint and scoped-CSS audit passed. The original R-06/R-07
review probes and remapped-Enter/global-Escape checks passed separately.
Gateway code is unchanged from the 870-test passing run at `926dc6d`;
the recorded live-simulator evidence remains attached to `84024bf`/`926dc6d`.
The integration comparison must differ from `f221763` only by this record and
the removal of the environment symlink. No controller restart or deployment
is part of the merge.

**Next milestone: `feat/twp-guided-setup`.** Keep these acceptance items open:

- M-05: a recorded desktop/simulated-touch workflow, with the active frame,
  edited datum and physical plane alignment visible together. Explain why a
  plane is stale and offer the appropriate recovery without silently moving
  the head or redefining the setup. Distinguish restoring an existing plane
  from capturing or measuring a new one; explain pointer-only hold actions.
- M-06: representative browser worker acknowledgement and control/camera
  responsiveness measurements, with any accepted limits recorded explicitly.
- M-07: assemble the next milestone's final acceptance record, retaining the
  original hashes of evidence carried forward from unchanged components.

Guide manual setup first. Probe-assisted datum setting in a known plane and
measurement of an unknown plane are distinct follow-up capabilities, each
requiring explicit coordinate/offset semantics and motion validation. Their
