...
The TWP (tilted work plane) machine — `lcnc_suite_sim_twp.ini`, the `twp/`
remap fork, `hallib/core_sim_6.hal`, and the `machine-xyzacb-trsrn/` viewer
model — lives on the **`feat/twp` branch**, split off development
2026-08-24 (see docs/decisions.md). Check out or worktree that branch to
run it; its own copy of this README carries the full setup instructions,
including the one-time `halcompile --install` precondition.
