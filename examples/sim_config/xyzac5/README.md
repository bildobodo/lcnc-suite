...
`install.sh` installs this example on both fresh installations and upgrades,
with its own offsets and tool table. On an already installed suite, run
`python3 scripts/install_examples.py` from the repository root while LinuxCNC
is stopped, then launch:

```sh
linuxcnc ~/linuxcnc/configs/lcnc_suite_sim/lcnc_suite_sim_5axis_xyzac.ini
```
